...
describe('NutritionBadge', () => {
  ...
  it('applies protein styling for high-protein badge', () => {
    render(<NutritionBadge tag="high-protein" />);
    const badge = screen.getByText('P');
    expect(badge).toHaveClass('bg-protein-light', 'text-protein-text');
  });

  it('applies fiber styling for high-fiber badge', () => {
    render(<NutritionBadge tag="high-fiber" />);
    const badge = screen.getByText('F');
    expect(badge).toHaveClass('bg-fiber-light', 'text-fiber-text');
  });
});
