...
export function NutritionGoalsSection({ items, store, onAddToList }: NutritionGoalsSectionProps) {
  ...
  return (
    <div className="mb-3">
      <div className="flex items-center justify-between px-3 py-2.5 bg-gradient-to-r from-both-light to-both-light/50 rounded-lg">
        <span className="font-semibold text-sm text-both-text">For Your Nutrition Goals</span>
        <span className="text-xs font-semibold text-both-text bg-white px-2.5 py-1 rounded-full shadow-sm">
          {items.length}
        </span>
      </div>
        ...
        {items.map(item => {
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 p-3 rounded-sm transition-all hover:shadow-md ${style.bg} border-l-[3px] ${style.border}`}
            >
              ...
              <button
                ...
                className={`w-9 h-9 flex items-center justify-center ${style.text} ${style.hoverText} ${style.hoverBg} hover:shadow-sm rounded-full transition-all active:scale-90`}
